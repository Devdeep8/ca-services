  ...
  useEffect(() => {
    async function fetchWorkspaces() {
      ...
      try {
        ...
        setCurrentWorkspace(data.currentWorkspace || (data.workspaces && data.workspaces[0]) || null)
      } catch (error) {
        console.error(error)
                            ...
                            <CommandList>
                                ...
                                <CommandGroup>
                                    {workspaces.map((ws) => (
                                        ...
                                        >
                                            <Check
                                                className={`mr-2 h-4 w-4 ${currentWorkspace?.id === ws.id ? 'opacity-100' : 'opacity-0'}`}
                                            />
                                            {ws.name}
                                        </CommandItem>
                                    ))}
          ...
          <div className="p-4 border-t mt-auto">
            <DropdownMenu>
                ...
                <DropdownMenuContent className="w-56" align="end" forceMount>
                    ...
                    <DropdownMenuItem onClick={() => signOut({ callbackUrl: '/sign-in' })} className="text-red-600 focus:text-red-600 focus:bg-red-50">
      ...
      <Dialog open={isAddWorkspaceOpen} onOpenChange={setIsAddWorkspaceOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <form onSubmit={handleAddWorkspace}>
            ...
            <DialogFooter>
              <Button type="button" variant="secondary" onClick={() => setIsAddWorkspaceOpen(false)}>
                Cancel
              </Button>
              <Button type="submit" disabled={isSubmitting || !newWorkspaceName}>
